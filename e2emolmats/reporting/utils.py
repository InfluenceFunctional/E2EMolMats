...
def read_lammps_pe_traj(filename):
    f = open(filename, "r")
    text = f.read()
    lines = text.split('\n')
    f.close()
    ...
    for ind, line in enumerate(lines):
        ...
        if line == '\n':
            ...
            if len(splitline[1].split('.')) == 1 and mol_counter > n_mols:  # integer
                if len(frame_data) > 0:
                    frames[frame_num] = frame_data
                a, b = splitline
                frame_num = int(a)
                n_mols = int(b)
                frame_data = np.zeros((n_mols, 1))
                mol_counter = 1
            else:
                try:
                    mol_num, pe = np.asarray(splitline).astype(float)
                except ValueError:
                    if int(mol_num) == n_mols:
                        pass
                    else:
...
def get_melt_point(temperature, energy_traj):
    assert False, "Redo this based on CoM deviation"
...
def crystal_stability_analysis(combined_df):
    ...
    for ind, row in combined_df.iterrows():
        ...
        start_time_index = len(row['com_mobility']) - (row['sampling_end_index'] - row['sampling_start_index'])
        ...
        calculated_stability = 1 - mobility_fraction
    ...
    for p_ind, polymorph in enumerate(present_polymorphs):
        ...
        for d_ind, defect_rate in enumerate(present_defect_rates):
            ...
            for t_ind, temp in enumerate(np.unique(temperature)):
                ...
                stab = np.array([np.mean(sta[-5:]) for sta in stability])[good_inds]
...
def run_initial_melt_succeeded(row):
    ...
    if row['melt_indices'] is not None:
        ...
        if fraction_melted > 0.7:
            ...
            melted = False
    else:  # nothing was supposed to melt, so we skip the check (this is a positive-only test)
        melted = False
    """
    import plotly.graph_objects as go
...
def com_mobility(com_traj: np.ndarray,
                 trailing_length: int = 5):
    mobility = np.zeros_like(com_traj)
    for ind in range(trailing_length):
        mobility[ind] = np.var(com_traj[:ind], axis=0)

    for ind in range(trailing_length, len(com_traj) + 1):
        mobility[ind - trailing_length:ind] = np.var(com_traj[ind - trailing_length:ind], axis=0)

    return mobility.sum(-1)
